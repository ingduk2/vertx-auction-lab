...
로컬 맥북에서 k6로 WebSocket 동시 접속 테스트 (Chapter 6-4)

**테스트 조건**
- 환경: 로컬 맥북, IntelliJ `bootRun` (개발 모드)
- 각 VU가 WebSocket 연결 후 **1초마다 랜덤 금액(1,000 ~ 11,000) 입찰**
- 경매 시간: 40초
- `bid_fail`: 현재 최고가보다 낮은 입찰 (에러 아님, 정상 경매 로직)

  | VUs        | p(95) latency | ws_connecting avg | ws_msgs_received | interrupted |
  |------------|---------------|-------------------|------------------|-------------|
  | 10         | 3ms           | 8ms               | 430              | 0           |
  | 100        | 8ms           | 20ms              | 4,500            | 0           |
  | 500        | 12ms          | 31ms              | 25,000           | 0           |
  | 1,000      | 29ms          | 70ms              | 49,000           | 0           |
  | 2,000      | 64ms          | 51ms              | 104,000          | 0           |
  | 5,000      | 30ms          | 158ms             | 235,000          | 0           |
  | **10,000** | **49ms**      | **345ms**         | **470,000**      | **0**       |

> **C10K 달성!** 10,000명 동시 WebSocket 접속, p(95) 49ms, interrupted 0
> Vert.x Event Loop가 단일 스레드로 수천 개의 WebSocket 연결을 처리한 결과
>
> 실제 운영 환경(네트워크, jar 빌드, JVM 튜닝)에서는 더 높은 성능을 기대할 수 있음
> 본 테스트는 로컬 개발 환경에서의 WebSocket 연결 처리 능력 확인 목적
